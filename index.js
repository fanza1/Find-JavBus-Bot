...
bot.onText(/\/av (.+)/, async (msg, match) => {
    ...
    try {
        ...
        if(result.cover) {
        // console.log('发送封面-开始', result.cover);
...
async function parseHtml(id) {
    ...
    response = await http({method: 'get', url, headers: {'referer': siteUrl + id}});
    // console.log('新的请求结果', response.data);
    ...
    if ($tr.length > 0) {
        for (let i = 0; i < $tr.length; i++) {
            ...
            result.magnet.push({link: decodeURI($a.attr('href').trim()), size: $a.text().trim(), dateTime:$a1.text().trim()});
        }
    }
    //console.log('最终结果',result);
    return result;
